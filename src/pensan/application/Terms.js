...
import Currency from '../dictionary/Currency';
...
const Terms = ({ onBack }) => {
  ...
  const [toggleStates, setToggleStates] = useState({
    calculationFromPremium: false
  });

  // Navigation handlers
  const handleBackToMain = () => setCurrentView('main');
  const handleOpenFrequencyPayment = () => setCurrentView('frequencyPayment');
  const handleOpenCurrency = () => setCurrentView('currency');
  ...
  if (currentView === 'currency') {
    return <Currency onBack={handleBackToMain} onSelect={(value) => handleDictionaryValueSelect('currency', value)} />;
  }

  return (
    <div data-layer="Terms" className="Terms" style={{width: 393, height: '100vh', minHeight: '100vh', background: 'white', overflow: 'hidden', flexDirection: 'column', alignItems: 'center', display: 'flex', position: 'relative'}}>
      {/* Fixed Header */}
      <div data-layer="Header" data-show-indicator="false" data-type="Main" className="Header" style={{width: 393, height: 85, background: 'white', overflow: 'hidden', borderBottom: '1px #F8E8E8 solid', justifyContent: 'space-between', alignItems: 'center', display: 'flex', position: 'fixed', top: 0, left: '50%', transform: 'translateX(-50%)', zIndex: 1000}}>
        ...
        <div data-layer="Title" className="Title" style={{flex: '1 1 0', height: 85, justifyContent: 'center', alignItems: 'center', gap: 10, display: 'flex'}}>
          <div data-layer="Screen Title" className="ScreenTitle" style={{flex: '1 1 0', textBoxTrim: 'trim-both', textBoxEdge: 'cap alphabetic', textAlign: 'center', color: 'black', fontSize: 16, fontFamily: 'Inter', fontWeight: '500', wordWrap: 'break-word'}}>Условия</div>
        </div>
      ...
      <div 
        ...
        data-state={toggleStates.calculationFromPremium ? "pressed" : "not_pressed"} 
        ...
        onClick={() => handleToggleClick('calculationFromPremium')}
      >
        <div data-layer="Text container" className="TextContainer" style={{flex: '1 1 0', height: 19, paddingTop: 20, paddingBottom: 20, paddingRight: 16, justifyContent: 'flex-start', alignItems: 'center', gap: 10, display: 'flex'}}>
          <div data-layer="LabelDiv" className="Labeldiv" style={{flex: '1 1 0', justifyContent: 'center', display: 'flex', flexDirection: 'column', color: 'black', fontSize: 16, fontFamily: 'Inter', fontWeight: '500', wordWrap: 'break-word'}}>Расчет от премии</div>
        </div>
        <div data-layer="Switch container" className="SwitchContainer" style={{width: 85, height: 85, position: 'relative', background: '#FBF9F9', overflow: 'hidden'}}>
          <div data-svg-wrapper data-layer="tui-switches" className="TuiSwitches" style={{left: 26, top: 35, position: 'absolute'}}>
            ...
            <rect width="32" height="16" rx="8" fill={toggleStates.calculationFromPremium ? "black" : "#E0E0E0"}/>
            <circle cx={toggleStates.calculationFromPremium ? "24" : "8"} cy="8" r="6" fill="white"/>
            </svg>
          </div>
        </div>
      </div>
      {renderDictionaryButton('frequencyPayment', 'Периодичность оплаты', handleOpenFrequencyPayment, !!dictionaryValues.frequencyPayment)}
      {renderDictionaryButton('currency', 'Валюта', handleOpenCurrency, !!dictionaryValues.currency)}
          {renderInputField('insurancePremium', 'Страховая премия', activeField === 'insurancePremium', !!fieldValues.insurancePremium)}
          {renderInputField('term', 'Срок', activeField === 'term', !!fieldValues.term)}
          {renderInputField('insuranceAmount', 'Страховая сумма', activeField === 'insuranceAmount', !!fieldValues.insuranceAmount)}
        </div>
      </div>
      ...
      <div data-layer="Button Container" className="ButtonContainer" style={{width: 393, height: 100, paddingLeft: 16, paddingRight: 16, paddingTop: 16, paddingBottom: 16, background: 'white', borderTop: '1px #F8E8E8 solid', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 10, display: 'flex', position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', zIndex: 1000}}>
        <div data-layer="Button" data-state="pressed" className="Button" style={{width: 361, height: 68, background: '#E0E0E0', overflow: 'hidden', borderRadius: 8, justifyContent: 'space-between', alignItems: 'center', display: 'inline-flex'}}>
          <div data-layer="Button Text" className="ButtonText" style={{flex: '1 1 0', textBoxTrim: 'trim-both', textBoxEdge: 'cap alphabetic', textAlign: 'center', color: 'black', fontSize: 16, fontFamily: 'Inter', fontWeight: '500', wordWrap: 'break-word'}}>Рассчитать</div>
        </div>
      </div>
    </div>
  );
